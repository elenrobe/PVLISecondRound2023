...
export default class GameScene extends Phaser.Scene {
	...
	preload() {
		...
		this.load.spritesheet('explosion','assets/sprites/explosion.png,', {frameWidth: 14,frameHeight: 14});
	...
	create(){
		this.lose=this.sound.add('lose', {volume: 0.1}, {loop: false});
		...
		this.anims.create({
			key: 'exp',
			frames: this.anims.generateFrameNames('asteroid', {start:2, end:3}),
			frameRate:4,
			repeat:-1
		});
		...
		this.physics.add.overlap(this.player, this.asteroids,()=>{
			...
			this.time.addEvent({delay:1250, callback: ()=>{this.startScene()},callbackScope:this, loop:false});
			this.lose.play();
